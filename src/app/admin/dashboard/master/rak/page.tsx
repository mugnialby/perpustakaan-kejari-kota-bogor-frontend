...
export default function MasterRak() {
    ...
    const API_URL = "http://192.168.50.52:8080/api/master/racks/";
    ...
    const handleSubmitRack = async (e: React.FormEvent) => {
        ...
        const rackData = {
            rackName: formData.get("rackName"),
        };

        try {
            if (isEditing && editingRack) {
                // PUT (or PATCH) for update
                await axios.put(`${API_URL}${editingRack.id}`, rackData, {
                    headers: { "Content-Type": "application/json" },
                });
                showToast("success", "Data rak berhasil diperbarui");
            } else {
                // POST for new rack
                await axios.post(API_URL, rackData, {
                    headers: { "Content-Type": "application/json" },
                });
    ...
    const handleDelete = async (rack: any) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await axios.patch(`${API_URL}${rack.id}`, { status: "N" });
    ...
    const totalPages = Math.ceil(filteredRacks.length / recordsPerPage);
    const startIndex = (currentPage - 1) * recordsPerPage;
    const currentRacks = filteredRacks.slice(
        startIndex,
                ...
                <table className="w-full border-collapse text-sm min-w-[600px]">
                    <thead className="bg-green-100 text-gray-900">
                        <tr>
                            ...
                            <th className="px-3 py-2 text-left w-full">Nama Rak</th>
                            ...
                            {currentRacks.length > 0 ? (
                                currentRacks.map((rack, idx) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-3 py-2">{rack.rackName}</td>
                ...
                <div className="flex flex-wrap justify-between items-center mt-4 text-sm gap-3">
                    ...
                    <div className="flex gap-1">
                        ...
                        <span className="text-gray-600 px-3 py-1">
                            ...
                            {Math.min(startIndex + recordsPerPage, filteredRacks.length)} dari{" "}
                            {filteredRacks.length} data
                        </span>
                        ...
                        >
                            ...
                            <form onSubmit={handleSubmitRack} className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        name="rackName"
                                        defaultValue={editingRack?.rackName || ""}
                                        required
                                ...
                                {/* <div>
                                    ...
                                    <input
                                        name="rackLocation"
                                        defaultValue={editingRack?.rackLocation || ""}
                                        required
                                        className="w-full border px-3 py-2 rounded-lg text-black"
                                    />
                                </div> */}
